...
def test_stt_concat_key_digits_split_two_questions():
    """Whisper пишет «Ключ 114» вместо «Ключ 1-1-4»; второй ответ «Ключ 214» → «2-1-4»."""
    t = (
        "Билет номер 4. Ключ 114. Как происходит передача сигнала. "
        "Ответ про нейроны и слои. "
        "Ключ 214. Какие модели используются. "
        "Ответ про модели обучения."
    )
    keys = ["1-1-4", "2-1-4"]
    parts, err, try_llm = segmentation_service.segment_transcript_to_keys(t, keys)
    assert err is None
    assert try_llm is False
    assert "нейроны" in parts["1-1-4"].lower() or "слои" in parts["1-1-4"].lower()
    assert "модели" in parts["2-1-4"].lower()


def test_stt_space_separated_key_digits():
    """Whisper иногда пишет «1 1 4» вместо «1-1-4»."""
    t = (
        "Ключ 1 1 4. Ответ первый. "
        "Ключ 2 1 4. Ответ второй."
    )
    keys = ["1-1-4", "2-1-4"]
    parts, err, try_llm = segmentation_service.segment_transcript_to_keys(t, keys)
    assert err is None
...
def test_real_whisper_two_keys_no_vopros_word():
    """Реальный транскрипт Whisper: студент говорит «Ключ 114» и «Ключ номер 1, 2, 4»
    без слова «вопрос» — оба ответа должны сегментироваться."""
    t = (
        "Билет номер 4. Ключ 114. Как происходит передача сигнала между нейронами "
        "в искусственной нейросети? Ответ. Передача сигнала между нейронами происходит "
        "в зависимости от слоев нейронной сети, их весовой значимости. Сперва данные "
        "поступают через входные или слои, где приобретают связь и вес. В значимости "
        "информации переходим на последующие слои. Связь при высоком весе суммируется "
        "значением. На внутренних слоях или скрытых слоях происходит обработка, анализ "
        "информации, которая при достаточном весе суммируется при возникновении ошибок "
        "или неверных суждений, например, среднеквадратическая ошибка или кроссентропия, "
        "сигнал не способен перейти на следующий уровень. Также измеряются пороговые "
        "значения для перехода на следующий слой. После выходит на выходной слой, "
        "где принимается окончательное решение, то бишь ответ. "
        "Ключ номер 1, 2, 4. Как изменялись подходы к ИИ с появлением машинного обучения? "
        "Машинное обучение позволило ИИ перейти с строго соблюдаемых алгоритмов на обучение "
        "с использованием разных техник, для того чтобы ИИ мог самостоятельно решать "
        "различные задачи. То есть идет обучение с учителем, когда работа производится "
        "с базами данных. Ответ окончен."
    )
    keys = ["1-1-4", "1-2-4"]
    parts, err, try_llm = segmentation_service.segment_transcript_to_keys(t, keys)
    assert err is None
    assert try_llm is False
    assert "передача сигнала" in parts["1-1-4"].lower()
    assert "машинное обучение" in parts["1-2-4"].lower()
    assert len(parts["1-1-4"]) > 100
    assert len(parts["1-2-4"]) > 100
